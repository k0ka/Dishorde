...
const channelIds = typeof config.channel === 'object' ? config.channel : [config.channel.toString()];
...
function parseDiscordCommand(msg, mentioned) {
    ...
    if (cmd.startsWith("SETCHANNEL")) {
        msg.channel.send(":x: Setchannel is not supported, use config instead.");
        return;
    }
    ...
    if (cmd.startsWith("EXEC")) {
        ...
        if (!msg.member.permissions.has("MANAGE_GUILD") || channels.findIndex(channel => msg.guild !== channel.guild) !== -1) {
    ...
    if (channels.includes(msg.channel) || msg.channel.type === "DM") {
        // 7d!info
...
if (!config["skip-discord-auth"]) {
    ...
    client.on("messageCreate", (msg) => {
        ...
        } else if (channels.includes(msg.channel) && msg.channel.type === "GUILD_TEXT") {
            const msgText = `[${getGuildName(msg)}][${msg.member.displayName}] ${msg.cleanContent}`;
            handleMsgToGame(msgText);
